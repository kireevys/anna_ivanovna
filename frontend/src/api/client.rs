...
impl ApiClient {
    ...
    async fn parse_response<T>(&self, response: gloo_net::http::Response) -> Result<T, ApiError>
    where
        T: DeserializeOwned,
    {
    ...
    pub async fn get_history(&self, from: Option<Cursor>) -> Result<Page<StorageBudget>, ApiError> {
    ...
    pub async fn add_income(&self, request: AddIncomeRequest) -> Result<Budget, ApiError> {
        let url = self.build_url("add_income")?;
        let response = Request::post(url.as_str())
            .json(&request)
            .map_err(|e| ApiError::Serialization(format!("Failed to serialize request: {e}")))?
            .send()
            .await
            .map_err(|e| ApiError::Network(format!("Request failed: {e}")))?;
