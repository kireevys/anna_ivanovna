...
fn plan_distribute_from_file() {
    ...
    let result_path = "bench_result.json";
    let base = env::current_dir().expect("Failed to get current directory");
    ...
    let weights = plan.try_into().unwrap();
    let d = distribute(&weights, &income).expect("whaaaat???");
